...
class Backend {
    ...
    function __construct() {
        if($_SERVER['SERVER_NAME']=='camera.com'){
    ...
    /* manufacture */
    function insertManu($manu_name,$manu_alias,$description,$image_url,$is_hot,$display_order,$meta_title,$meta_description,$meta_keyword,$hidden) {
        try{    
            $user_id = $_SESSION['user_id'];
            $time = time();            
    ...
    function updateManu($id,$manu_name,$manu_alias,$description,$image_url,$is_hot,$display_order,$meta_title,$meta_description,$meta_keyword,$hidden) {
       try{              
            $user_id = $_SESSION['user_id'];   
            $time = time();    
            $sql = "UPDATE manu
    ...
    function getListProduct($product_code,$product_name,$cate_type_id,$cate_id,$is_new,$is_saleoff,$is_hot,$trangthai,$offset,$limit) {
        try{            
            ...
            $sql = "SELECT * FROM product WHERE (cate_type_id = $cate_type_id OR $cate_type_id = -1) AND (cate_id= $cate_id OR $cate_id = -1) AND (is_hot = $is_hot OR $is_hot = -1) AND (is_new= $is_new OR $is_new = -1) AND (trangthai= $trangthai OR $trangthai = -1) ";                     
